...
const TURNS_FILE = "./sicbo_turns.json";
let userTurns = {};

if (fs.existsSync(TURNS_FILE)) {
  try { userTurns = JSON.parse(fs.readFileSync(TURNS_FILE, "utf8")); } catch {}
}

function saveTurns() {
  try { fs.writeFileSync(TURNS_FILE, JSON.stringify(userTurns, null, 2)); } catch {}
}

function checkAndResetTurns() {
  const now = Date.now();
  const resetInterval = 30 * 60 * 1000;
  for (const uid in userTurns) {
    if (now - userTurns[uid].lastReset >= resetInterval) {
      userTurns[uid].count = 0;
      userTurns[uid].lastReset = now;
    }
  }
}

function canPlay(uid) {
  checkAndResetTurns();
  if (!userTurns[uid]) {
    userTurns[uid] = { count: 0, lastReset: Date.now() };
    saveTurns();
    return true;
  }
  return userTurns[uid].count < 15;
}

function incrementTurn(uid) {
  if (!userTurns[uid]) {
    userTurns[uid] = { count: 0, lastReset: Date.now() };
  }
  userTurns[uid].count++;
...
function getRemainingTurns(uid) {
  checkAndResetTurns();
  if (!userTurns[uid]) {
    userTurns[uid] = { count: 0, lastReset: Date.now() };
    saveTurns();
  ...
  return 15 - userTurns[uid].count;
}

function getResetTime(uid) {
  if (!userTurns[uid]) return 0;
  const resetTime = userTurns[uid].lastReset + 30 * 60 * 1000;
...
module.exports = {
  ...
  onStart: async function ({ args, message, event, api }) {
    ...
    if (!sub || sub === "help") {
      return message.reply(UI(["🎲 SIC BO — 3 DES", "---", `${p}sicbo petit/grand <mise>`, `${p}sicbo total <mise> <4-17>`, `${p}sicbo triple <mise> [1-6/any]`, `${p}sicbo double <mise> [1-6/any]`, `${p}sicbo simple <mise> <1-6>`, `${p}sicbo combo <mise> <1-6> <1-6>`, `${p}sicbo bonus`, "---", "💎 Triple spécifique → x180", "💎 Any Triple → x30", "💎 Total Extreme (4/17) → x60", "---", `⏳ Limite: 15 tours / 30min`]));
    }

    const userMoney = await getUserCash(uid);
    if (sub === "balance") return message.reply(UI([`💰 Solde: ${await formatNumber(userMoney)}$`]));

    if (sub === "bonus") {
      await updateUserCash(uid, 200n);
      return message.reply(UI(["🎁 BONUS QUOTIDIEN", "---", "✨ +200$", `💰 Solde: ${await formatNumber(await getUserCash(uid))}$`]));
    }

    if (!canPlay(uid)) {
      const remaining = getResetTime(uid);
      const minutes = Math.floor(remaining / 60000);
      const seconds = Math.floor((remaining % 60000) / 1000);
      return message.reply(UI([
        "⛔ LIMITE ATTEINTE",
        "---",
        "Tu as utilisé tes 15 tours",
        `⏳ Prochain reset dans ${minutes}m ${seconds}s`,
        "---",
        "Les tours sont réinitialisés toutes les 30min"
      ]));
    }

    const betType = sub;
    const validTypes = ["petit","grand","total","triple","double","simple","combo"];
    if (!validTypes.includes(betType)) return message.reply(UI(["❌ Type de mise inconnu", `📝 ${p}sicbo help`]));

    const amount = await parseAmount(args[1]);
    if (amount <= 0n) return message.reply(UI(["❌ Montant invalide"]));
    if (amount > userMoney) return message.reply(UI(["❌ Fonds insuffisants", "---", `💰 Solde: ${await formatNumber(userMoney)}$`, `🎲 Mise: ${await formatNumber(amount)}$`]));

    let betValue = null;
    if (betType==="total"){betValue=parseInt(args[2]);if(isNaN(betValue)||betValue<4||betValue>17)return message.reply(UI(["❌ Total 4-17"]));}
    if (betType==="triple"||betType==="double"){betValue=args[2]||"any";if(betValue!=="any"&&(parseInt(betValue)<1||parseInt(betValue)>6))return message.reply(UI(["❌ Valeur 1-6 ou any"]));if(betValue!=="any")betValue=parseInt(betValue);}
    if (betType==="simple"){betValue=parseInt(args[2]);if(isNaN(betValue)||betValue<1||betValue>6)return message.reply(UI(["❌ Valeur 1-6"]));}
    ...
    let betDisplay="";
    if(betType==="total")betDisplay=`Total=${betValue}`;else if(betType==="triple")betDisplay=`Triple ${betValue}`;else if(betType==="double")betDisplay=`Double ${betValue}`;else if(betType==="simple")betDisplay=`Num ${betValue}`;else if(betType==="combo")betDisplay=`${betValue[0]}+${betValue[1]}`;else betDisplay=betType==="petit"?"Petit (4-10)":"Grand (11-17)";

    const [fBet,fNew,fWin,fRemaining]=await Promise.all([formatNumber(amount),formatNumber(newBalance),formatNumber(winAmount),formatNumber(remainingTurns)]);
    await message.reply(UI(["🎲 SIC BO — RESULTAT", "---", `🎲 Des: ${dice.map(d=>DICE_EMOJI[d]).join(" ")}`, `📊 Total: ${sum}${isTriple?" ✦ TRIPLE!":""}`, "---", `📋 Mise: ${betDisplay}`, `💰 Montant: ${fBet}$`, "---", win?`🎉 GAIN — +${fWin}$ (x${payout})`:`💀 PERDU — -${fBet}$`, `💳 Solde: ${fNew}$`, "---", `🎯 Tours restants: ${fRemaining}/15`]));
    ...
    if(imageMode){
      try{
        ...
        await message.reply({body:"🎲 Carte résultat:",attachment:fs.createReadStream(imgPath)});
